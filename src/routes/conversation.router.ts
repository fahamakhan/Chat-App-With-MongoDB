...
export class ConversationRouter {
    ...
    private async get(req, res) {
        try {
            ...
            const contacts = await this.conversationController.get(search, type);
            contacts === null
                ? res.status(404).send(
    ...
    private async getBy(req, res) {
        try {
            const { userId, maulanaId } = req.query as any;
            const contact = await this.conversationController.getBy(userId, maulanaId);
            contact === null
                ? res.status(404).send(
                    new NotFoundError(`No record found`, {
                        message: `No record found`,
